...
const clubReferralSchema = zod.object({
  body: zod.object({
    ...
    instagram: zod.preprocess(
      sanitizeUrl,
      zod.string().url("Invalid Instagram URL"),
    ),
    linkedIn: zod.preprocess(
      sanitizeUrl,
      zod.string().url("Invalid LinkedIn URL"),
    ),
    portfolio: zod.preprocess(
      sanitizeUrl,
      zod.string().url("Invalid Portfolio URL"),
    ),
  }),
});
