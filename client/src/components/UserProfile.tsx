...
//                    user.roles?.includes('STUDENT_PRESIDENT') ? '🎓 Student President' :
//                    user.roles?.includes('DONOR') || user.roles?.includes('PREMIUM_DONOR') ? '❤️ Donor' :
//                    '🎓 Student'}
//                 </span>
//               </div>
//             </div>
...
import { can } from '../rbac/engine';
import { Permissions } from '../rbac/permissions';
import FacultyVerificationCard from './profile/FacultyVerificationCard'; // <-- NEW IMPORT
  ...
  return (
    <div className="min-h-screen bg-dreamxec-cream relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 relative z-10">
        ...
        {/* Profile Card */}
        <div className="card-pastel-offwhite rounded-xl border-5 border-dreamxec-navy shadow-pastel-card p-6 sm:p-8 mb-8">
          <div className="card-tricolor-tag"></div>
          ...
          <div className="flex items-center gap-6 mt-4">
            ...
            <div className="flex-1">
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {/* Dynamically render all RBAC roles */}
                {user.roles?.map((role) => (
                  <span key={role} className="inline-flex items-center gap-2 px-3 py-1 bg-dreamxec-beige border-2 border-dreamxec-navy rounded-lg text-dreamxec-navy font-bold text-sm font-display">
                    {role === 'ADMIN' ? '🛡️ Admin' :
                     role === 'STUDENT_PRESIDENT' ? '🎓 Student President' :
                     role === 'FACULTY' ? '🎓 Faculty' :
                     role === 'DONOR' || role === 'PREMIUM_DONOR' ? '❤️ Donor' :
        ...
        {/* GRID LAYOUT FOR VERIFICATION & SETTINGS */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          
          {/* LEFT COLUMN: Verification & Info */}
          <div className="lg:col-span-5 flex flex-col gap-6">
            
            {/* Faculty Verification Card Component */}
            <FacultyVerificationCard />

            {/* Secure & Encrypted Info Card */}
            <div className="card-pastel rounded-xl p-5 border-3 border-dreamxec-navy shadow-pastel-saffron bg-white">
              <div className="flex items-start gap-3">
                <div className="icon-pastel-container w-10 h-10 p-2 flex-shrink-0 bg-yellow-100 rounded-lg border-2 border-dreamxec-navy">
                  <span className="text-xl flex items-center justify-center">🔒</span>
                </div>
                <div>
                  <h3 className="text-lg font-black text-dreamxec-navy font-display uppercase tracking-tight mb-1">
                    Secure & Encrypted
                  </h3>
                  <p className="text-dreamxec-navy/70 font-sans text-sm leading-relaxed font-bold">
                    Your identity and bank details are encrypted and stored securely. We never share your sensitive information with third parties.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="lg:col-span-7">
            <div className="card-pastel-offwhite rounded-xl border-5 border-dreamxec-navy shadow-pastel-card p-6 sm:p-8">
              ...
              {hasBankDetails && !isEditing && savedBankDetails ? (
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  {error && (
                    <div className="p-4 bg-red-100 border-2 border-red-600 text-red-700 rounded-lg font-sans font-bold">
                      {error}
                    </div>
                  )}
                  ...
                  {/* Account Holder Name */}
                  <div>
                    <label className="block text-sm font-black uppercase tracking-widest text-dreamxec-navy mb-2">
                      Account Holder Name <span className="text-red-600">*</span>
                    </label>
                    <div className="relative">
                      <UserIcon className="absolute left-4 top-1/2 transform -translate-y-1/2 text-dreamxec-navy opacity-60 w-5 h-5" />
